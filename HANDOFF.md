...
## v1.3.7で入ったもの
- 取込タブの下に「要整理」セクション。状態が未確認、またはカテゴリがその他の店を並べ、
  その場でカテゴリ変更・営業中・閉店を切り替えられる。ここでの修正は self_visit 1.0 で記録するため、
  以後の取込で上書きされない
- Bonsai資料のMarkdownに公式サイトと来店サマリ（来店回数・最終来店・平均評価）を追加。
  RAG側で「よく行く店」「最近行った店」が引けるようになる
